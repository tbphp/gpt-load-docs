  ...
  return (
    ...
    >
      ...
      <ul>
        ...
        <li>
          <strong>测试连接</strong>——用这一条凭据发一次最小的真实上游请求，
          当场看它还能不能用
        </li>
        <li>
      ...
      <Notice label="测试连接会真的花钱（一点点）" tone="amber">
        测试<b>发的是真实上游请求</b>，会产生少量费用。
        它<b>不改变调度或拉黑状态</b>，所以可以放心用来确认「这把密钥到底还有没有效」。
        <br />
        结果有三种：<b>测试通过</b>、<b>测试未通过</b>（会说明是密钥无效、
        上游限流还是模型不可用），以及<b>暂时无法判断</b>——
        最后这种通常是超时或该渠道不支持探测，不代表密钥有问题。
        <br />
        如果测的这把凭据当前正被拉黑，测试通过后会直接问你要不要恢复它，
        不用再去健康页操作一遍。
      </Notice>
